...
class TrueWallet {
	...
	public function RequestLoginOTP () {
		...
		$result = $this->request("GET", "/mobile-auth-service/v1/password/login/otp/", array(
			...
			"timestamp" => $timestamp,
            "signature" => hash_hmac("sha1", implode("|", array(strval($this->config["username"]), hash("sha256", hash("sha256", substr($timestamp, 4)).hash("sha256", strval($this->config["username"]).strval($this->config["password"]))), strval($this->config["device_id"]), $timestamp)), "?"),
	...
	public function DraftTransferP2P ($mobile_number, $amount) {
		if (!isset($this->config["access_token"])) return false;
		$hash = implode("|",
							array(rtrim($this->mobile_api_endpoint, "/")."/transfer-composite/v2/p2p-transfer/draft-transactions/",
							strval($this->config["access_token"]),
							number_format(str_replace(",", "", strval($amount)), 2, ".", ""),
							str_replace(array("-", " "), "", strval($mobile_number))
						));
		$this->calculate_singature($hash);	
	...
	public function Promptpay ($promptpay, $amount) {
		if (!isset($this->config["access_token"])) return false;
		$hash = implode("|",
							array(rtrim($this->mobile_api_endpoint, "/")."/transfer-composite/v1/promptpay/inquiries",
							strval($this->config["access_token"]),
							"QR",
							number_format(str_replace(",", "", strval($amount)), 2, ".", ""),
							strval($promptpay)
		...
		$this->calculate_singature($hash);	
		return $this->request("POST", "/transfer-composite/v1/promptpay/inquiries", array(
            "Authorization" => strval($this->config["access_token"]),
            "Signature" => $this->remote_key_value,
            "X-Device" => $this->remote_key_id,
			"X-Geo-Position" => "lat=; lng=",			
			...
			"input_method" => "QR",	
			"amount" => number_format(str_replace(",", "", strval($amount)), 2, ".", ""),
			"to_proxy_value" => strval($promptpay),	
		));
	}
